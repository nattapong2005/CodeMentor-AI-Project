...
import { FaBars, FaBook, FaUser, FaRegCheckSquare } from "react-icons/fa";
  ...
  return (
    ...
    >
      ...
      <nav className="mt-4 flex-1">
        {navItems.map((item) => {
          // 3. (ปรับปรุง) ตรวจสอบ isActive ด้วย startsWith เพื่อรองรับ Sub-routes
          // เช่น /teacher/classrooms/123 จะยังคงไฮไลท์เมนู "ห้องเรียน"
          ...
          const content = (
            <>
              ...
              <span
                className={`whitespace-nowGrap overflow-hidden transition-all duration-300 ml-3 ${isOpen ? "opacity-100" : "opacity-0 w-0"}`}
              >
                {item.title}
              </span>
            </>
